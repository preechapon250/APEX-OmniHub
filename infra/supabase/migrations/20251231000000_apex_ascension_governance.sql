...
-- ============================================================================
-- PHYSICAL AI ORCHESTRATION: Temporal-Linked RLS
-- ============================================================================

-- Physical device registry table
CREATE TABLE IF NOT EXISTS public.physical_devices (
    ...
    security_registry_status text DEFAULT 'unknown',
    web3_verified boolean DEFAULT false,
    web3_owner text,
    temporal_execution_id text,
...
CREATE POLICY "physical_devices_temporal_rls" ON public.physical_devices
FOR ALL TO authenticated USING (true)
WITH CHECK (temporal_execution_id IS NOT NULL);

CREATE POLICY "physical_devices_service_role" ON public.physical_devices
FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE INDEX IF NOT EXISTS idx_physical_devices_temporal ON public.physical_devices(temporal_execution_id);
CREATE INDEX IF NOT EXISTS idx_physical_devices_web3 ON public.physical_devices(web3_owner) WHERE web3_verified = true;
