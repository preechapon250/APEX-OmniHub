...
await page.screenshot({ path: join(screenshotsDir, 'screenshot-homepage-light.png'), fullPage: true });
console.log('Captured: Homepage (Light Theme)');

// Screenshot 2: Homepage header close-up (light theme)
await page.screenshot({
  path: join(screenshotsDir, 'screenshot-header-light.png'),
...
await page.screenshot({
  path: join(screenshotsDir, 'screenshot-header-dark.png'),
  clip: { x: 0, y: 0, width: 1920, height: 100 }
});
